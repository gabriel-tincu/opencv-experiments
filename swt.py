        ...
        for x in range(edges.shape[1]):
            for y in range(edges.shape[0]):
                if edges[y, x] > 0:
                    ...
                    while True:
                        i += 1
                        cur_x = np.floor(x + grad_x * i)
                        cur_y = np.floor(y + grad_y * i)
                ...
                if sw_point < np.Infinity and sw_point > 0:
                    ...
                    for neighbor in neighbors:
                        ...
                        try:
                            ...
                            continue
                        if label_n > 0 and (sw_n / sw_point < swt_ratio_threshold or sw_point / sw_n < swt_ratio_threshold):
...
if __name__ == '__main__':
    path = '/home/gabi/workspace/ocropy/corpus/570a6493a310cb6bf27e019a_5f223673-7abd-42c7-8005-18e9a949b8b1.jpg'
    mask = SWTScrubber.scrub(path)
    s = set()
    s.update(mask.flatten())
    print(s)
    import shutil
    shutil.copy(path, '/tmp/orig.jpg')
    cv2.imwrite('/tmp/mask.jpg', flat_mask(mask))